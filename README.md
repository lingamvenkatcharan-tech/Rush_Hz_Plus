...
Hz+는 청각장애인과 난청인이 일상생활에서 화재 경보, 사이렌, 유리 파손, 비명, 총성, 울음소리, 차량 경적 등 위험한 소리를 네트워크 없이 즉시 인지할 수 있도록 돕는 **온디바이스 AI 기반 안전 앱**입니다.

단순한 소리 감지를 넘어, 위험 수준에 따른 다중 감각 경고(진동·시각·음성)와 보호자 긴급 알림 연동을 통해 사용자가 위험 상황을 인지하고 대응할 수 있도록 지원하는 것을 목표로 합니다.
...
| 기능 | 설명 |
|------|------|
| **온디바이스 위험 소리 분류** | YAMNet 기반 TFLite 모델로 7개 위험 클래스 온디바이스 추론 (평균 지연 약 52ms) |
| **다중 감각 경고** | L1(주의) ~ L3(긴급) 단계별 진동 패턴, 플래시, TTS 음성 안내 |
| **보호자 자동 연동** | L3 발생 시 전화, SMS 위임, 앱 푸시를 통해 보호자에게 알림 전달 (다중 채널 기반) |
...
- **결과**: 다양한 채널(전화, SMS, 앱 푸시)을 병행하여 보호자 알림 전달 성공률을 개선
...
- **전체 정확도 (검증 데이터셋 기준)**: **97.56%**

> 설계 철학: 안전성 우선 – 위험 신호를 놓치는 것(False Negative)을 최소화.
(데이터 불균형으로 인해 일부 클래스에서 정밀도는 낮을 수 있음)
